...
const codingAgentDir = join(repoRoot, "packages/nova");
...
try {
	...
	if (checkOnly) {
		if (!existsSync(outputPackageJsonPath) || !existsSync(outputLockfilePath)) {
			console.error("packages/nova/install-lock is missing generated files.");
			console.error("Run: npm run install-lock:nova");
			process.exit(1);
		}
		...
		if (currentPackageJson !== packageJsonContent || currentLockfile !== lockfileContent) {
			console.error("packages/nova/install-lock is out of date.");
			console.error("Run: npm run install-lock:nova");
			process.exit(1);
		}
		console.log("packages/nova/install-lock is up to date.");
	} else {
		...
		const platformPackageCount = Object.values(installLock.packages).filter((entry) => entry.os || entry.cpu || entry.libc)
			.length;
		console.log(
			`Wrote packages/nova/install-lock/package.json and package-lock.json (${packageCount} packages, ${platformPackageCount} platform-specific).`,
		);
	}
} catch (error) {
