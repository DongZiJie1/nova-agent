...
const packageDir = join(repoRoot, "packages", "nova");
...
const agentDirEnvName = "NOVA_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "NOVA_STARTUP_BENCHMARK";

function printHelp() {
	console.log(`Usage:
  node scripts/profile-nova-node.mjs [options]

Profiles nova startup with the runtime selected below:
- npm run profile:tui     -> builds packages/nova and profiles TUI startup with Node
- npm run profile:rpc     -> builds packages/nova and profiles RPC startup with Node
...
Options:
  ...
  --agent-dir <dir>      Use a specific NOVA_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force NOVA_OFFLINE=1 / NOVA_SKIP_VERSION_CHECK=1
...
async function runBuild() {
	process.stdout.write("Building packages/tui, packages/ai, packages/agent, and packages/nova...\n");
...
async function main() {
	...
	if (runtime === "bun") {
		process.stdout.write(
			`Using Bun runtime with ${options.mode === "rpc" ? "packages/nova/src/cli.ts --mode rpc" : "packages/nova/src/cli.ts"}\n`,
		);
	}
