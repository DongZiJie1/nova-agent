...
import { parseSessionEntries, type SessionMessageEntry } from "../packages/nova/src/core/session-manager.ts";
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".nova/agent/sessions");
	...
	const globalAgentsMd = join(homedir(), ".nova/agent/AGENTS.md");
