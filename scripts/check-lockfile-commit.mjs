...
const allowValue = process.env.NOVA_ALLOW_LOCKFILE_CHANGE ?? process.env.PI_ALLOW_LOCKFILE_CHANGE;
...
if (allowed) {
	console.error("package-lock.json is staged; NOVA_ALLOW_LOCKFILE_CHANGE is set, allowing commit.");
	process.exit(0);
}
...
console.error("  - regenerate/check nova shrinkwrap if release deps changed");
...
console.error("  NOVA_ALLOW_LOCKFILE_CHANGE=1 git commit ...  (legacy alias: PI_ALLOW_LOCKFILE_CHANGE=1)");
process.exit(1);
