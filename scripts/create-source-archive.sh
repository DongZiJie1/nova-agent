...
package_version="$(git show "${commit}:packages/nova/package.json" | node -p 'JSON.parse(require("fs").readFileSync(0, "utf8")).version')"
...
required_paths=(
    ...
    "packages/nova/src/utils/image-resize-worker.ts"
    "packages/nova/src/core/export-html/template.css"
)
...
if grep -Eq '(^|/)node_modules/|(^|/)packages/nova/binaries/' "$manifest"; then
