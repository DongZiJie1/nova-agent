...
try {
	...
	if (checkOnly) {
		if (!existsSync(shrinkwrapPath)) {
			console.error("packages/nova/npm-shrinkwrap.json is missing.");
			console.error("Run: npm run shrinkwrap:nova");
			process.exit(1);
		}
		const current = readFileSync(shrinkwrapPath, "utf8");
		if (current !== content) {
			console.error("packages/nova/npm-shrinkwrap.json is out of date.");
			console.error("Run: npm run shrinkwrap:nova");
			process.exit(1);
		}
		console.log("packages/nova/npm-shrinkwrap.json is up to date.");
	} else {
		writeFileSync(shrinkwrapPath, content);
		const packageCount = Object.keys(shrinkwrap.packages).length - 1;
		const platformPackageCount = Object.values(shrinkwrap.packages).filter((entry) => entry.os || entry.cpu || entry.libc).length;
		console.log(
			`Wrote packages/nova/npm-shrinkwrap.json (${packageCount} packages, ${platformPackageCount} platform-specific).`,
		);
	}
} catch (error) {
