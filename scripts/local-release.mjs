...
const packages = [
	{ directory: "packages/ai", name: "@dongzijie1/pi-ai" },
	{ directory: "packages/tui", name: "@dongzijie1/pi-tui" },
	{ directory: "packages/agent", name: "@dongzijie1/pi-agent-core" },
	{ directory: "packages/storage/sqlite-node", name: "@dongzijie1/pi-storage-sqlite-node" },
	{ directory: "packages/nova", name: "@dongzijie1/nova" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "nova-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `nova-${platform}.zip` : `nova-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createNovaShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "nova.cmd"))) {
			writeFileSync(join(installDirectory, "nova.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\nova.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "nova.ps1"), '& "$PSScriptRoot/node_modules/.bin/nova.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "nova.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\nova.exe" %*\r\n');
		writeFileSync(join(installDirectory, "nova.ps1"), '& "$PSScriptRoot/node_modules/.bin/nova.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "nova"), join(installDirectory, "nova"));
}
...
if (rootPackageJson.name !== "nova-monorepo") {
...
if (!options.skipInstall) {
	...
	createNovaShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createNovaShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `nova-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "nova.exe" : "nova")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "nova.cmd" : "nova")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "nova.cmd" : "nova")} --help`);
	}
}
