...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		...
		await writeManifest(root, "packages/coding-agent", {
			name: "@dongzijie1/pi-coding-agent",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			...
			private: true,
			dependencies: {
				"@dongzijie1/pi-coding-agent": "^1.0.0",
		...
		assert.equal(evalsManifest.dependencies["@dongzijie1/pi-coding-agent"], "^2.0.0");
		...
		assert.equal(generatedManifest.dependencies["@dongzijie1/pi-coding-agent"], "^1.0.0");
