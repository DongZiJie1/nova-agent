...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "@dongzijie1/pi-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the `TUI` constructor option, `setShowHardwareCursor(true)`, or `NOVA_HARDWARE_CURSOR=1` (the legacy `PI_HARDWARE_CURSOR` name is still read). The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { CombinedAutocompleteProvider } from "@dongzijie1/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@dongzijie1/pi-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@dongzijie1/pi-tui";
import type { Component } from "@dongzijie1/pi-tui";
...
Set `NOVA_TUI_WRITE_LOG` (the legacy `PI_TUI_WRITE_LOG` name is still read) to capture the raw ANSI stream written to stdout.

```bash
NOVA_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
