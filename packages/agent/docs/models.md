...
- Do not migrate nova `ModelRegistry` yet.
...
The end state for nova: AuthStorage is deleted; its capabilities map onto a `CredentialStore` implementation plus composition.
...
| stored `api_key` (with `$ENV`/`!command` via `resolveConfigValue`) | stored `ApiKeyCredential`; config-value resolution happens at `read` in nova's adapter/decorator (command execution stays app policy) |
...
`@dongzijie1/pi-ai/compat` preserves the old global API surface until the nova migration deletes it. New code never imports it.
...
nova (and the interim agent package) switch imports of these symbols from `@dongzijie1/pi-ai` to `@dongzijie1/pi-ai/compat` (import-path-only change) and are otherwise untouched until the ModelManager migration.

Extension grace period: the nova extension loader (jiti aliases + Bun `virtualModules`) resolves the `@dongzijie1/pi-ai` ROOT specifier to the compat entrypoint. Existing user extensions using the old global API (`complete`, `getModel`, `registerApiProvider`, ...) keep working at runtime without changes; they break only when compat is removed at the ModelManager migration, with a migration guide in the changelog. Typechecking is the nudge: editors resolve the root to the slim core types, so extension sources that typecheck must import old globals from `/compat` — which is what the repo example extensions demonstrate.
...
1. Main `@dongzijie1/pi-ai` import is core-only.
...
## nova next phase (not this pass)

nova builds providers in layers and binds them per session:
...
- nova does not use `AgentHarness` yet; `AgentSession` still drives the low-level `Agent` with a `streamFn`.
- nova still uses legacy `AuthStorage` + `ModelRegistry` and imports old global pi-ai APIs through `@dongzijie1/pi-ai/compat`.
...
- [x] Callback-server flows race a `manual_code` prompt, aborted through `AuthPrompt.signal` once the flow settles. The public `oauth` subpath retains only nova extension compatibility types.
...
- [x] Browser smoke (entry now imports old globals from `/compat`) + shrinkwrap checks green. Internal old-global imports switched to `/compat` already (42 files in agent/nova/examples; vitest configs alias `/compat` to src; spawn-CLI tests resolve workspace dist, so `packages/ai` + `packages/agent` dists were rebuilt).
...
### Phase 7 — nova bridge (minimal)

- [x] Switch old-global imports to `@dongzijie1/pi-ai/compat` (landed with Phase 5; compat is a superset so the switch was path-only). Extension loader resolves the pi-ai root to compat as the runtime grace period.
- [x] Everything else originally sketched here is gated on nova actually streaming through a `Models` instance — nova's `AgentSession` drives the low-level `Agent` via `streamFn`, not the harness — and moved to Phase 9.
...
- [x] `packages/nova/CHANGELOG.md`: `### Changed` entry for extension authors — runtime unaffected (loader resolves the pi-ai root to compat), typecheck nudges to `/compat` or the new API; removal happens later with a migration guide.
...
### Phase 9 — nova on Models + CredentialStore (in scope)

nova replaces AuthStorage and ModelRegistry's internals with `FileCredentialStore` + a `MutableModels` collection. AgentSession itself stays (AgentHarness adoption is pi 2.0); only its model/auth substrate swaps. Layering is strictly one-directional:
...
3. [ ] Add `FileCredentialStore` in nova.
...
4. [ ] Add runtime override overlay for nova policy.
   ...
   - Runtime overrides must apply even to OAuth-capable providers; every provider registered in nova must retain or gain an `apiKey` auth slot so the overlay is meaningful.
...
7. [x] Extension OAuth adapter.
   - Keep only the legacy callback/credential declarations required by nova `ProviderConfig.oauth`.
...
8. [ ] Rebuild nova `ModelRegistry` over `MutableModels`.
