...
```bash
npm install @dongzijie1/pi-ai
```

TypeBox exports are re-exported from `@dongzijie1/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@dongzijie1/pi-ai';
import { builtinModels } from '@dongzijie1/pi-ai/providers/all';
...
import { openaiProvider } from '@dongzijie1/pi-ai/providers/openai';
import { openrouterProvider } from '@dongzijie1/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@dongzijie1/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@dongzijie1/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@dongzijie1/pi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@dongzijie1/pi-ai';
...
API-key credentials use the same discriminator as Nova's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@dongzijie1/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@dongzijie1/pi-ai';
...
```typescript
import { builtinImagesModels } from '@dongzijie1/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@dongzijie1/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@dongzijie1/pi-ai';
import { openAICompletionsApi } from '@dongzijie1/pi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@dongzijie1/pi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@dongzijie1/pi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@dongzijie1/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@dongzijie1/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@dongzijie1/pi-ai';
import { anthropicProvider } from '@dongzijie1/pi-ai/providers/anthropic';
import { openaiProvider } from '@dongzijie1/pi-ai/providers/openai';
import { googleProvider } from '@dongzijie1/pi-ai/providers/google';
...
- `@dongzijie1/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@dongzijie1/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@dongzijie1/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@dongzijie1/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@dongzijie1/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@dongzijie1/pi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@dongzijie1/pi-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `NOVA_CACHE_RETENTION` (the legacy `PI_CACHE_RETENTION` name is still read), and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @dongzijie1/pi-ai login              # interactive provider selection
npx @dongzijie1/pi-ai login anthropic    # login to specific provider
npx @dongzijie1/pi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@dongzijie1/pi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@dongzijie1/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@dongzijie1/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@dongzijie1/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@dongzijie1/pi-ai`.
...
#### 6. Coding Agent Integration (`../nova/`)
