...
const originalPiPackageDir = process.env.NOVA_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.NOVA_PACKAGE_DIR;
	} else {
		process.env.NOVA_PACKAGE_DIR = originalPiPackageDir;
	}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const packageDir = join(scopeDir, "nova");
	mkdirSync(packageDir, { recursive: true });
	tempDir = prefix;
	process.env.NOVA_PACKAGE_DIR = packageDir;
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@mariozechner", "nova");
	mkdirSync(packageDir, { recursive: true });
	...
	process.env.NOVA_PACKAGE_DIR = packageDir;
	setExecPath(
		join(root, ".pnpm", "@mariozechner+nova@0.0.0", "node_modules", "@mariozechner", "nova", "dist", "cli.js"),
	);
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "nova");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.NOVA_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "nova", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@earendil-works+nova@0.67.68\\node_modules\\@earendil-works\\nova\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@dongzijie1/nova")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @dongzijie1/nova",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@dongzijie1/nova")).toBeUndefined();
		expect(getUpdateInstruction("@dongzijie1/nova")).toBe(
			"Update @dongzijie1/nova using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@dongzijie1/nova");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@dongzijie1/nova"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @dongzijie1/nova`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@dongzijie1/nova", undefined, {
			packageName: "@dongzijie1/nova",
			installSpec: "@dongzijie1/nova@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@dongzijie1/nova@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @dongzijie1/nova@1.2.3`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@mariozechner/nova", undefined, "@new-scope/pi");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/nova && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/pi`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@mariozechner/nova"],
					display: `npm --prefix ${prefix} uninstall -g @mariozechner/nova`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@dongzijie1/nova", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @dongzijie1/nova`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@dongzijie1/nova")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @dongzijie1/nova",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@dongzijie1/nova"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @dongzijie1/nova",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"pnpm remove -g @mariozechner/nova && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/pi",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@mariozechner/nova"],
					display: "pnpm remove -g @mariozechner/nova",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/pi"],
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@dongzijie1/nova";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@earendil-works", "nova");
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.NOVA_PACKAGE_DIR = storePackageDir;
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display: "yarn global remove @mariozechner/nova && yarn global add --ignore-scripts @new-scope/pi",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@mariozechner/nova"],
					display: "yarn global remove @mariozechner/nova",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"bun uninstall -g @mariozechner/nova && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/pi",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@mariozechner/nova"],
					display: "bun uninstall -g @mariozechner/nova",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/pi"],
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@dongzijie1/nova")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@dongzijie1/nova")).toContain("the install path is not writable");
	});
});
