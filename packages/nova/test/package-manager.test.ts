...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.NOVA_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.NOVA_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should resolve project paths relative to .pi", async () => {
			const extDir = join(tempDir, ".nova", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".nova"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".nova", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".nova", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".nova", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".nova", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".nova", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json nova.extensions in extensions setting", async () => {
			// Create a package with nova.extensions in package.json
			...
			// Should find the extensions declared in package.json nova.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .nova/agent skills", async () => {
		...
		it("should use the project .pi dir as baseDir for project .pi skills", async () => {
			const projectBaseDir = join(tempDir, ".nova");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".nova", "agent");
		...
		it("should dedupe user skill entries when ~/.nova/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .pi auto-discovery", async () => {
			...
			const skillDir = join(tempDir, ".nova", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with nova manifest", async () => {
			const pkgDir = join(tempDir, "my-nova-package");
		...
		it("should keep legacy pi manifest entries with leading tilde package-relative", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".nova", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .pi settings base", () => {
			...
			const rel = relative(join(tempDir, ".nova"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in nova manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			// User-level resources are discovered under HOME (~/.agents/*), so point
			// HOME at tempDir to keep the developer's own skills out of the result.
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
			...
			try {
				...
				settingsManager.setProjectPackages([
					{
						source: relative(join(tempDir, ".nova"), pkgDir),
						autoload: false,
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect legacy package.json pi.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".nova", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".nova", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".nova", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".nova", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".nova", "npm", "node_modules", "example");
