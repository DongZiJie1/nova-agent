...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".nova", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".nova", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".nova", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".nova"), { recursive: true });
			writeFileSync(join(cwd, ".nova", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".nova"), { recursive: true, force: true });
