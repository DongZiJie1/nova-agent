...
> **Placement for /reload:** Put extensions in `~/.nova/agent/extensions/` (global) or `.nova/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
