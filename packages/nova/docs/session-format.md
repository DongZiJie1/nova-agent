...
```
~/.nova/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.nova/agent/sessions/`.

Nova also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, nova uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([nova-agent](https://github.com/DongZiJie1/nova-agent)):
- [`packages/nova/src/core/session-manager.ts`](https://github.com/DongZiJie1/nova-agent/blob/main/packages/nova/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/nova/src/core/messages.ts`](https://github.com/DongZiJie1/nova-agent/blob/main/packages/nova/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/DongZiJie1/nova-agent/blob/main/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/@dongzijie1/nova/dist/` and `node_modules/@dongzijie1/pi-ai/dist/`.
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if Nova-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `nova.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `nova.setSessionName()` in extensions.
