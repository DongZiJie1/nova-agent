> nova can create themes. Ask it to build one for your setup.
...
Nova loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.nova/agent/themes/*.json`
- Project: `.nova/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `nova.themes` entries in `package.json`
...
On first run, nova detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.nova/agent/themes
vim ~/.nova/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/DongZiJie1/nova-agent/main/packages/nova/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, nova reloads it automatically for immediate visual feedback.
...
Nova uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, nova falls back to the nearest approximation.
