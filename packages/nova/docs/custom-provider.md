...
Extensions can register custom model providers via `nova.registerProvider()`. This enables:
...
Extensions can register either a complete pi-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Nova composes `models.json` overrides above registered native providers.
...
export default function (nova: ExtensionAPI) {
  nova.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  nova.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. nova waits for the factory before startup continues, so the provider is available during interactive startup and to `nova --list-models`.
...
Use `nova.unregisterProvider(name)` to remove a provider that was previously registered via `nova.registerProvider(name, ...)`:
...
// Later, remove it
nova.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map nova levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
nova.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@dongzijie1/pi-ai";
...
Credentials are persisted in `~/.nova/agent/auth.json`:
...
- [anthropic.ts](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, nova can recover automatically by compacting the conversation and retrying. This recovery only kicks in if nova recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of nova's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/DongZiJie1/nova-agent/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message nova does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase nova recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before nova tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what nova checks. With this in place, nova will:
...
- Match a provider-specific pattern, not nova's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of nova's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/DongZiJie1/nova-agent/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps nova thinking levels to provider/model-specific values; null marks a level unsupported. */
   ...
   * context window here makes providers reject every request. Nova warns about a
   * `maxTokens` that does not fit the context window, and recovers from a value
   * that exceeds the endpoint's own cap by retrying once with the cap the endpoint
