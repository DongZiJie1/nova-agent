...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@dongzijie1/nova";

export default function (nova: ExtensionAPI) {
	nova.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
