 ...
 *
 * Usage:
 *   nova -e ./ssh.ts --ssh user@host
 *   nova -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (nova: ExtensionAPI) {
	nova.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	nova.on("session_start", async (_event, ctx) => {
		...
		const arg = nova.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
