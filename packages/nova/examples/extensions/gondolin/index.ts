 ...
 *
 * Runs nova's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 *
 * Setup:
 *   cd packages/nova/examples/extensions/gondolin
 *   npm install --ignore-scripts
 *
 * Usage:
 *   cd /path/to/project
 *   nova -e /path/to/nova/packages/nova/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (nova: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `nova ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	nova.on("session_shutdown", async (_event, ctx) => {
		const activeVm = vm;
		vm = undefined;
		vmStarting = undefined;
		if (!activeVm) return;
	...
	nova.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
