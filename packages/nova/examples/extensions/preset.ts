 ...
 * - ~/.nova/agent/presets.json (global)
 * - <cwd>/.nova/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `nova --preset plan` - start with plan preset
...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@dongzijie1/nova";
import type { Api, Model } from "@dongzijie1/pi-ai";
import { Container, Key, type SelectItem, SelectList, Text } from "@dongzijie1/pi-tui";
...
export default function presetExtension(nova: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: nova.getThinkingLevel(),
				tools: nova.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			nova.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = nova.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				nova.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await nova.setModel(originalState.model);
				}
				nova.setThinkingLevel(originalState.thinkingLevel);
				nova.setActiveTools(originalState.tools);
			} else {
				nova.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	nova.registerShortcut(Key.ctrlShift("u"), {
	...
	nova.on("before_agent_start", async (event) => {
	...
	nova.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = nova.getFlag("preset");
	...
	nova.on("turn_start", async () => {
		if (activePresetName) {
			nova.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
