...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@dongzijie1/nova";
import { StringEnum } from "@dongzijie1/pi-ai";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@dongzijie1/pi-tui";
...
export default function (nova: ExtensionAPI) {
	nova.on("session_start", async (_event, ctx) => reconstructState(ctx));
	nova.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	nova.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	nova.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	nova.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			nova.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						nova.appendEntry(SAVE_TYPE, getBoardDetails());
