import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "@dongzijie1/nova";
import type { Component, EditorTheme, TUI } from "@dongzijie1/pi-tui";
...
export default function (nova: ExtensionAPI) {
	...
	nova.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await nova.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = nova.getThinkingLevel();
