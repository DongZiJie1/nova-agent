...
async function resolveGitHubRepo(nova: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await nova.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (nova: ExtensionAPI): void {
	nova.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(nova, ctx.cwd);
		if (!resolvedRepo.ok) {
