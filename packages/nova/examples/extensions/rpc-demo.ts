...
export default function (nova: ExtensionAPI) {
	...
	nova.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "nova RPC Demo (new session)" : "nova RPC Demo");
	...
	nova.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	nova.on("turn_end", async (_event, ctx) => {
	...
	nova.registerCommand("rpc-input", {
	...
	nova.registerCommand("rpc-editor", {
	...
	nova.registerCommand("rpc-prefill", {
