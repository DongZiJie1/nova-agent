 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit nova cleanly.
 */
...
export default function (nova: ExtensionAPI) {
	// Register a /quit command that cleanly exits nova
	nova.registerCommand("quit", {
		description: "Exit nova cleanly",
	...
	nova.registerTool({
		...
		description: "Complete a task and exit nova",
	...
	nova.registerTool({
		...
		description: "Deploy the application and exit nova",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await nova.exec("npm", ["run", "deploy", params.environment], { signal });
