 ...
 *
 * Demonstrates nova.sendUserMessage() for sending user messages from extensions.
 * Unlike nova.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (nova: ExtensionAPI) {
	...
	nova.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			nova.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	nova.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				nova.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	nova.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				nova.sendUserMessage(args, { deliverAs: "followUp" });
