...
export default function (nova: ExtensionAPI) {
	nova.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await nova.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[nova] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await nova.exec("git", ["add", "-A"]);
		const { code: commitCode } = await nova.exec("git", ["commit", "-m", commitMessage]);
