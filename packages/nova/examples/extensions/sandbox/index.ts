 ...
 *
 * Usage:
 * - `nova -e ./sandbox` - sandbox enabled with default/config settings
 * - `nova -e ./sandbox --no-sandbox` - disable sandboxing
...
import { type BashOperations, CONFIG_DIR_NAME, createBashTool, getAgentDir } from "@dongzijie1/nova";
...
export default function (nova: ExtensionAPI) {
	...
	nova.on("session_start", async (_event, ctx) => {
		const noSandbox = nova.getFlag("no-sandbox") as boolean;
