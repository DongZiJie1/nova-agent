...
import { Markdown, type MarkdownTheme } from "@dongzijie1/pi-tui";
...
import { getLatestNovaRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestNovaRelease>>;
	try {
		latestRelease = await getLatestNovaRelease(VERSION);
	} catch (error: unknown) {
