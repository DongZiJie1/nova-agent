<p align="center">
  <a href="https://github.com/DongZiJie1/nova-agent">
    <img alt="nova logo" src="assets/nova.png" width="128">
  </a>
</p>
<p align="center">
...
For each built-in provider, nova maintains a list of tool-capable models. Configured provider catalogs refresh automatically; run `nova update --models` to force an immediate refresh. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
Nova also supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage downloads and loaded models with `/llama`, then select a loaded model with `/model`. See [docs/llama-cpp.md](docs/llama-cpp.md) for setup and usage.

See [docs/providers.md](docs/providers.md) for other provider setup instructions.

**Custom providers & models:** Add providers via `~/.nova/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.nova/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so Nova can receive the follow-up shortcut.
...
Sessions auto-save to `~/.nova/agent/sessions/` organized by working directory.

```bash
nova -c                  # Continue most recent session
nova -r                  # Browse and select from past sessions
nova --no-session        # Ephemeral mode (don't save)
nova --name "my task"    # Set session display name at startup
nova --session <path|id> # Use specific session file or ID
nova --fork <path|id>    # Fork specific session file or ID into a new session
```
...
| Location | Scope |
|----------|-------|
| `~/.nova/agent/settings.json` | Global (all projects) |
| `.nova/settings.json` | Project (overrides global) |
...
Nova loads `AGENTS.md` (or `CLAUDE.md`) at startup from:
- `~/.nova/agent/AGENTS.md` (global)
...
Replace the default system prompt with `.nova/SYSTEM.md` (project) or `~/.nova/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.nova/agent/skills/my-skill/SKILL.md -->
# My Skill
...
TypeScript modules that extend Nova with custom tools, commands, keyboard shortcuts, event handlers, and UI components.

```typescript
export default function (nova: ExtensionAPI) {
  nova.registerTool({ name: "deploy", ... });
  nova.registerCommand("stats", { ... });
  nova.on("tool_call", async (event, ctx) => { ... });
}
```

The default export can also be `async`. Nova waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `nova.registerProvider()`.
...
- Make Nova look like Claude Code
...
nova install git:git@github.com:user/repo@v1  # tag or commit
nova install https://github.com/user/repo
...
nova update --extensions                  # update packages only
...
**No sub-agents.** There's many ways to do this. Spawn nova instances via tmux, or build your own with [extensions](#extensions), or install a package that does it your way.
...
`nova config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `nova update` never prompts for project trust.
...
In print mode, Nova also reads piped stdin and merges it into the initial prompt:
...
# High thinking level
nova --thinking high "Solve this complex problem"
```
