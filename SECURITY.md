...
Nova and also where the boundaries are.

In general Nova is a coding agent that runs locally within the security boundary
...
Nova treats the local user account and files writable by that account as inside
the same trust boundary as the Nova process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Nova configuration, they can generally influence Nova or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Nova grants that write access or crosses
an operating-system privilege boundary.

Nova relies on users installing trustworthy extensions and loading trustworthy
skills and only to use Nova within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in Nova or another package in
...
- Local code execution or sandboxing behavior (the Nova coding agent intentionally does not have a sandbox)
- Behavior of Nova extensions or skills installed by the user
...
- Public internet exposure of a Nova installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.nova`,
  `~/.nova/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Nova itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the Nova coding agent.
...
For example, a report showing that malicious contents written to a trusted Nova
configuration file cause Nova to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Nova.  For exposed-secret reports,
include evidence that the credential is owned by this project's maintainer or
grants access to infrastructure operated by it.
